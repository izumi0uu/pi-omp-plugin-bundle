...
import { registerToolDisplayOverrides } from "./tool-overrides.ts";
import { disposeAll, resetDisposed } from "./disposable.ts";
import { registerThinkingLabeling } from "./thinking-label.ts";
import registerNativeUserMessageBox from "./user-message-box-native.ts";
import {
...
export default function toolDisplayExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("tool-display", {
    ...
    handler: async (args, ctx) => {
      const { runToolDisplayCommandHandler } = await import("./config-modal.ts");
