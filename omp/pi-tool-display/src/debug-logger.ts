...
import { toRecord } from "./tool-metadata.ts";
import { resolvePiAgentDir } from "./agent-dir.ts";

const RUNTIME_CONFIG_DIR = join(resolvePiAgentDir(), "extensions", "pi-tool-display");
const DEFAULT_DEBUG_CONFIG_FILE = join(RUNTIME_CONFIG_DIR, "config.json");
const DEFAULT_DEBUG_DIR = join(RUNTIME_CONFIG_DIR, "debug");
