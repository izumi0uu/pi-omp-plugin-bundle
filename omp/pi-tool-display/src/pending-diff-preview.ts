...
function safeRealpathWithMissingPath(path: string): string {
	...
	return join(safeRealpath(existingPath), ...missingSegments);
}
...
function resolveWorkspaceReadPath(cwd: string, rawPath: string): { resolvedPath: string; error?: string } {
	...
	const candidatePath = resolvePreviewPath(cwd, rawPath);
	const resolvedPath = safeRealpathWithMissingPath(candidatePath);
