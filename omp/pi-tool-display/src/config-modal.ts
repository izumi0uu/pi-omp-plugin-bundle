...
import type { ToolDisplayCapabilities } from "./capabilities.ts";
import { getToolDisplayConfigPath } from "./config-store.ts";
import {
...
import { shortenPath } from "./render-utils.ts";
import type { InspectorSettingItem } from "./settings-inspector-modal.ts";
...
export async function openSettingsModal(ctx: ExtensionCommandContext, controller: ToolDisplayConfigController): Promise<void> {
	...
	const [{ ZellijModal }, { SplitPaneInspectorModal }] = await Promise.all([
		import("./zellij-modal.ts"),
		import("./settings-inspector-modal.ts"),
	]);
